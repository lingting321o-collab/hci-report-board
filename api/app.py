...
from api import convert as convert_module
from api.convert import build_docx, parse_report
...
def build_actions(report):
    return [
        (
            "P0",
            "备份不可用风险",
            f"备份覆盖率 {report['backup_rate']}，已备份 {report['backup_done']}/{report['backup_total']} 台。",
            "客户危害：一旦核心业务虚拟机误删、损坏、勒索加密或存储故障，可能无法恢复到可用状态，直接造成业务长时间中断和数据丢失。",
            "立即扩容/清理备份空间，确认重要业务清单，补齐备份策略。",
            "立即",
            "FCE4D6",
        ),
        (
        ...
        (
            "P0",
            "虚拟机网卡类型异常",
            "检测到或需复核 rtl8139 虚拟机网卡。",
            "客户危害：相关虚拟机可能出现访问慢、丢包、业务连接中断或扫描结果不准确，客户会直观看到系统卡顿或服务不稳定。",
            "按 KB 调整网卡类型，变更前确认停机窗口和回退方案。",
            "1 周内",
            "FCE4D6",
        ),
        (
            ...
            "网络链路与交换配置",
            "检测到主机网口/物理网络相关告警。",
            "客户危害：链路冗余能力下降，网络收敛或端口异常时可能引发业务抖动、访问中断，甚至让多个业务同时受影响。",
            "检查网线和交换端口，将 HCI 对接端口设置为边缘端口或关闭不必要 STP。",
            "2 周内",
            "FFF2CC",
        ),
        (
            "P1",
            "平台告警与账号安全",
            "存在告警通知、密码策略、异常账号或端口暴露类问题。",
            "客户危害：故障发生时客户无法第一时间收到通知，账号滥用或弱口令可能导致平台被误操作、越权访问或安全事件扩大。",
            "启用告警通知，设置密码有效期，清理无效账号，关闭非必要端口。",
            "2 周内",
            "FFF2CC",
        ),
        (
            "P1",
            "虚拟机配置不规范",
            "存在虚拟机配置、快照或性能优化工具相关问题。",
            "客户危害：虚拟机性能和自愈能力不足，故障后恢复变慢；快照残留还可能导致后续备份、快照和存储维护失败。",
            "按业务重要性分批安装工具、开启保护项，并处理快照残留。",
            "2-4 周",
            "FFF2CC",
        ),
    ]
...
convert_module.build_actions = build_actions
...
def preview_report():
    ...
    try:
        ...
        return jsonify({
            ...
            "actions": [
                {"level": item[0], "name": item[1], "what": item[2], "impact": item[3], "do": item[4], "deadline": item[5]}
                for item in actions
            ],
        })
